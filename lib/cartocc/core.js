    ...
    for (var idx in this.rules) {
        ...
        for (fieldpath in rule['if']) {
            if (this.layerHasValue(layer, fieldpath, rule['if'][fieldpath])) {
                matched = true;
            } else if ((rule['if'][fieldpath] == "") && this.layerGetValue(layer, fieldpath)) {
                matched = true;
            } else {
                matched = false;
                break;
        ...
        if (matched) {
            for (fieldpath in rule.then) {
                ...
                if (value.match(/^replace/)) {
                  var tmpValue = this.layerGetValue(layer, fieldpath); 
                  this.setLayerValue(layer, fieldpath, eval("tmpValue." + value));
		} else {
...
C.prototype.layerGetValue = function(layer, fieldpath) {
    var flatLayer = this.flatenLayer(layer);
    return flatLayer[fieldpath];
}
